...
- **File transfer** — send files up to 200 MB via chunked data channel with progress tracking
...
Files are sent peer-to-peer over the WebRTC data channel — no server upload, no cloud storage.

**Send path (`rtc.js` → `sendFile`):**
1. The sender's browser reads the entire file into an `ArrayBuffer` via `file.arrayBuffer()`
2. A `file-start` JSON message is sent first, carrying the filename, MIME type, size, and total chunk count
3. The buffer is sliced into **16 KB chunks** (`CHUNK_SIZE = 16384`). Each chunk is prefixed with a 40-byte header: 36 bytes for the transfer UUID + 4 bytes for the chunk index
4. Chunks are sent sequentially. If `channel.bufferedAmount` exceeds **1 MB** (`MAX_BUFFERED`), sending pauses for 50 ms — this is the back-pressure mechanism that prevents overwhelming the data channel
5. A `file-end` JSON message is sent after the last chunk

**Receive path:**
1. `file-start` allocates a slot in `this._incoming` Map with a pre-sized chunks array
2. Each binary frame is decoded: header bytes identify the transfer ID and chunk index; the payload is stored at that index
3. `file-end` assembles all chunks into a `Blob`, creates an object URL, and fires the `file-complete` event
4. `chat.js` fetches the blob back as an `ArrayBuffer` and persists it to IndexedDB

**Memory usage:** The full file lives in RAM on both ends simultaneously during transfer (~200 MB peak for a max-size file). There is no streaming-to-disk path; the browser holds the whole file until IndexedDB write completes.

**Limit:** 200 MB (`MAX_FILE_SIZE = 200 * 1024 * 1024` in `chat.js`).
